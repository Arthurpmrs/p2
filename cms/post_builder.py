...
class PostBuilder:
    ...
    def build_post(self) -> Post:
        ...
        while True:
            ...
            if block_option == 0:
                ...
                block = TextBlock(order=order_counter, language=language, text=text)
                ...
                block = MediaBlock(
                    order=order_counter, language=language, media=media, alt=alt
                )
                self.blocks.append(block)
            else:
                print("Opção inválida.")
        ...
        post_content: PostContent = {"title": title, "body": self.blocks}

        post = Post(
            poster=self.poster,
            site=self.site,
            post_content_by_language={language: post_content},
    ...
    def select_media(self) -> Media | None:
